 ...
 * Time: 14:36
 */
class ObjectAutocompleteRegistrationNumberPredicateGenerator implements AutocompletePredicateGenerator {
    public Optional<SearchPredicate> generatePredicate(SearchPredicateFactory factory, ObjectAutocompleteFilter filter) {
        if (!ObjectUtils.isEmpty(filter.getRegistrationNumber())) {
            PredicateFinalStep res = factory.bool(b -> {
              SearchPredicate predicate = BasePredicateUtils.getWildcardPredicate(
                      factory,
                      IpObjectSearchFields.FIELD_OBJECT_REGISTRATION_NUMBER_TEXT,
                      "*" + filter.getRegistrationNumber() + "*", true);
              b.must(predicate);
            });
