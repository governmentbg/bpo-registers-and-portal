...
--changeset ggeorgiev:0028
--validCheckSum: 9:a164d1e7ee154ba173984c241a11bfc2
create view vw_object_relationships
...
SELECT orl.object_id2  AS object_id,
       ...
       concat(rt.inverse_relationship_name, ' ',
              (string_to_array(obj.id::text, '/'::text))[array_length(string_to_array(obj.id::text, '/'::text), 1)],
              CASE
                  WHEN obj.filing_date IS NOT NULL THEN concat(' / ', to_char(obj.filing_date::timestamp with time zone,
                                                                              'DD.MM.YYYY'::text))
                  ELSE ''::text
                  END) AS description,
       concat(rt.inverse_relationship_name_en, ' ',
              (string_to_array(obj.id::text, '/'::text))[array_length(string_to_array(obj.id::text, '/'::text), 1)],
              CASE
                  WHEN obj.filing_date IS NOT NULL THEN concat(' / ', to_char(obj.filing_date::timestamp with time zone,
                                                                              'DD.MM.YYYY'::text))
...
SELECT orl.object_id1  AS object_id,
       ...
       rt.relationship_name,
       rt.relationship_name_en,
       concat(rt.direct_relationship_name, ' ',
              (string_to_array(obj.id::text, '/'::text))[array_length(string_to_array(obj.id::text, '/'::text), 1)],
              CASE
                  WHEN obj.filing_date IS NOT NULL THEN concat(' / ', to_char(obj.filing_date::timestamp with time zone,
                                                                              'DD.MM.YYYY'::text))
                  ELSE ''::text
                  END) AS description,
       concat(rt.direct_relationship_name_en, ' ',
              (string_to_array(obj.id::text, '/'::text))[array_length(string_to_array(obj.id::text, '/'::text), 1)],
              CASE
                  WHEN obj.filing_date IS NOT NULL THEN concat(' / ', to_char(obj.filing_date::timestamp with time zone,
                                                                              'DD.MM.YYYY'::text))
